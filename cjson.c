...
int  recursion_limit;
static int _Py_CheckRecursionLimit;
...
decode_string(JSONData *jsondata)
{
	...
	if (has_utf8 && jsondata->all_unicode) {
		char *utf8_string;
		int utf8_len;


		if (PyString_AsStringAndSize(object, &(utf8_string), &(utf8_len)) == -1) {
			Py_DECREF(object);
			return NULL;
		}

		PyObject *new_object = PyUnicode_DecodeUTF8(utf8_string, utf8_len, NULL);
		Py_DECREF(object);
		object = new_object;
	}
	...
	if (object == NULL) {
		...
		if (type == NULL) {
			...
			if (PyErr_GivenExceptionMatches(type, PyExc_UnicodeError)) {
...
decode_number(JSONData *jsondata)
{
	...
	if (is_float)
		...
		object = PyLong_FromString(PyString_AS_STRING(str), NULL, 10);
		...
		*p++ = quote;
		for (i = 0; i < op->ob_size; i++) {
			...
			/*else if (c < ' ' || c >= 0x7f) {
				...
				p += 6;
			}*/ // shunf4: Do not escape special characters
			else
				*p++ = c;
		}
...
"objects everywhere (decode by utf-8, this is slower).") },
...
#define DL_EXPORT(RTYPE) extern "C" __declspec(dllexport) RTYPE
DL_EXPORT(void) initcjson()
{
	PyObject *m;
	recursion_limit = Py_DEFAULT_RECURSION_LIMIT;
	_Py_CheckRecursionLimit = Py_DEFAULT_RECURSION_LIMIT;

	m = Py_InitModule("cjson", cjson_methods);
